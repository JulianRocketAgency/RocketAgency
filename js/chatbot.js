...
function buildChatbot() {
  const html = `
    ...
    <div id="chat-window" role="dialog" aria-label="Rocket Agency chat">
      ...
      <div id="chat-whatsapp" style="display:none;padding:0 16px 12px;">
        <button onclick="openWhatsApp()" style="width:100%;background:#25D366;border:none;border-radius:3px;padding:12px;font-family:var(--font-ui,'Syne',sans-serif);font-size:.75rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:10px;transition:opacity .2s;" onmouseover="this.style.opacity='.88'" onmouseout="this.style.opacity='1'">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.123.553 4.116 1.52 5.845L.057 23.273a.75.75 0 00.92.92l5.429-1.463A11.945 11.945 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.891 0-3.666-.5-5.198-1.375l-.372-.215-3.863 1.041 1.041-3.862-.215-.372A9.944 9.944 0 012 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/></svg>
          📲 Druk hier — direct in contact met Julian
        </button>
      </div>
  ...
  if (role === 'user') {
    ...
    if (userMsgs >= 2) {
      ...
      if (waBtn && waBtn.style.display === 'none') {
        ...
        setTimeout(() => {
          appendMessage('bot', 'Wil je direct verder met Julian? Druk op de groene knop hieronder — dan sta je meteen in contact en neemt hij het gesprek van mij over. 👇');
        }, 600);
      }
    }
  }
